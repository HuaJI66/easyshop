...
public class GmCertUtil{
    ...
    private final static Logger logger = LoggerFactory.getLogger(GmCertUtil.class);
	...
	private static void initMiddleCert() {
        String path = GmUnionPayAutoConfiguration.getConfig().getMiddleCertPath();
        if(isEmpty(path)){
            logger.warn(GmUnionPayAutoConfiguration.SDK_MIDDLECERT_PATH + " is empty");
            return;
        }
	...
	private static void initRootCert() {
        String path = GmUnionPayAutoConfiguration.getConfig().getRootCertPath();
        if(isEmpty(path)){
            logger.warn(GmUnionPayAutoConfiguration.SDK_ROOTCERT_PATH + " is empty");
            return;
        }
	...
	private static void initVerifySignCerts(){
		String dir = GmUnionPayAutoConfiguration.getConfig().getValidateCertDir();
	...
	private static void initEncryptCert(){
        String path = GmUnionPayAutoConfiguration.getConfig().getEncryptCertPath();
        if(isEmpty(path)){
            logger.warn(GmUnionPayAutoConfiguration.SDK_ENCRYPTCERT_PATH + " is empty");
            return;
        }
        encryptCert = readGmPubCert(path);
        logger.info("加载加密证书==>" + path + (encryptCert != null ?"成功":"失败"));
	}

	private static void initPinEncryptCert(){
        String path = GmUnionPayAutoConfiguration.getConfig().getPinEncryptCertPath();
        if(isEmpty(path)){
            logger.warn(GmUnionPayAutoConfiguration.SDK_PINENCRYPTCERT_PATH + " is empty");
            return;
        }
        pinEncryptCert = readGmPubCert(path);
        logger.info("加载6.0统一支付产品pin加密证书==>" + path + (pinEncryptCert != null ?"成功":"失败"));
	}


//	public static PublicKey getEncryptCertPublicKey(){
//		return getEncryptKey(GmUnionPayAutoConfiguration.getConfig().getEncryptCertPath()).getPubKey();
//	}
//
//	public static String getEncryptCertId(){
//		return getEncryptKey(GmUnionPayAutoConfiguration.getConfig().getEncryptCertPath()).getCertId();
//	}
...
//		logger.debug("read sign key from [" + certFilePath + "] pwd [" + certPwd + "]");
    ...
    private static Key getSignKey() {
		String path = GmUnionPayAutoConfiguration.getConfig().getSignCertPath();
		String pwd = GmUnionPayAutoConfiguration.getConfig().getSignCertPwd();
    ...
    private static Key readGmPubCert(String path) {
    	...
    	try {
			...
			X509Certificate cert = (X509Certificate) cf.generateCertificate(in);
			if(cert == null) {
				logger.error("Load verify cert error, " + path + " has error cert content.");
				return null;
			}
			Key key = new Key();
			key.setCertId(cert.getSerialNumber().toString(10));
			key.setPubKey(cert.getPublicKey());
			// 打印证书加载信息,供测试阶段调试
			logger.info("[" + path + "][CertId=" + key.getCertId() +"]");
			return key;
		} catch (CertificateException e) {
			logger.error("LoadVerifyCert Error CertificateException", e);
		}catch (FileNotFoundException e) {
    ...
    private static boolean verifyCertificate(X509Certificate cert) {
		...
		if(GmUnionPayAutoConfiguration.getConfig().isIfValidateCNName()){
			// 验证公钥是否属于银联
	...
	public static int resetEncryptCertPublicKey(String strCert) {
        ...
        final String localCertPath = GmUnionPayAutoConfiguration.getConfig().getEncryptCertPath();
	...
	public static int resetPinEncryptCertPublicKey(String strCert) {
		...
		final String localCertPath = GmUnionPayAutoConfiguration.getConfig().getPinEncryptCertPath();
		if(isEmpty(localCertPath)){
			logger.error("未配置加密证书路径，无法执行此方法。");
			return -1;
		}

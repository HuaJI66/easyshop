...
//	}
    ...
    public static byte[] send(String urlStr, byte[] data, Map<String, String> reqHeader, String requestMethod){
        ...
        if(data != null)
        	logger.debug(requestMethod + " to [" + urlStr + "]: " + new String(data));
        else
        	logger.debug(requestMethod + " to [" + urlStr + "]");
        ...
        try {
            ...
            if(httpCode != HttpURLConnection.HTTP_OK){
                ...
                logger.error("HTTP RESP[" + httpCode + "]: " + new String(respBody, encoding));
            } else {
                respBody = read(httpURLConnection.getInputStream());
                logger.debug("HTTP RESP[" + httpCode + "]: " + new String(respBody, encoding));
            }
            return respBody;
        } catch (Exception e) {
	...
	public static boolean addSslConf(String tag,
        ...
        if(ssLSocketFactoryMap.containsKey(tag)) {
            logger.warn("addSslConf err: [" + tag + "] has been added");
            return false;
        }
		...
		try {
            ...
            logger.info("addSslConf succeed: [" + tag + "]" + (tmf == null ? "，但没验服务器证书哦" : "") + "。");
	        return true;
		} catch (Exception e) {
	...
	private static KeyStore loadKeyStore(String path, String pwd, String type) {
		...
		try {
			...
			logger.error("loadKeyStore Error", e);
		} finally {
			if (fis != null) {
				try {
					fis.close();

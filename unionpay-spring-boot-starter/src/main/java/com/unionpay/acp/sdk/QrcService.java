...
public class QrcService {
	...
	private final static Logger logger = LoggerFactory.getLogger(QrcService.class);
	...
	public static Map<String, String> sign(Map<String, String> reqData,String encoding) {
        return signByCertInfo(reqData, UnionPayAutoConfiguration.getConfig().getSignCertPath(), UnionPayAutoConfiguration.getConfig().getSignCertPwd(), encoding);
	}
	...
	public static Map<String, String> signByCertInfo(Map<String, String> reqData, String certPath,
        ...
        String signType = data.get(param_signType);
        String version = data.get(param_version);
        String reqType = data.get(param_reqType);
		...
		try {
	        if (VERSION_1_0_0.equals(version)) {
	        	...
	        	if (QRC_SIGNTYPE_SHA256WITHRSA.equals(signType)
					...
					logger.error("国密算法按要求必须通过加密机签名，本sdk不提供。");
					return data;
				}
	        }
            logger.error("未实现签名方法, version=" + version + ", signType=" + signType);
            return data;
        } catch (Exception e) {
	...
	public static boolean validate(Map<String, String> resData, String encoding) {
		...
		String signType = resData.get(param_signType);
        String version = resData.get(param_version);
        String reqType = resData.get(param_reqType);
        ...
        try {
            ...
            logger.error("未实现验签方法, version=" + version + ", signType=" + signType);
            return false;
        } catch (Exception e) {

...
public class AcpService {
	...
	public static Map<String, String> sign(Map<String, String> reqData, String encoding) {
        ...
        try{
            if (SIGNMETHOD_RSA.equals(signMethod)) {
                return signByCertInfo(data, UnionPayAutoConfiguration.getConfig().getSignCertPath(), UnionPayAutoConfiguration.getConfig().getSignCertPwd(), encoding);
            } else if (SIGNMETHOD_SHA256.equals(signMethod)) {
                return signBySecureKey(data, UnionPayAutoConfiguration.getConfig().getSecureKey(), encoding);
        ...
        try {
        	if(VERSION_5_0_1.equals(version) || VERSION_5_0_0.equals(version)){
        		if (SIGNMETHOD_RSA.equals(signMethod)) {
                     ...
                     data.put(param_signature, SDKUtil.signRsa(data, certPath, certPwd, encoding));
                     return data;
                }
        	} else if(VERSION_5_1_0.equals(version)){
        		if (SIGNMETHOD_RSA.equals(signMethod)) {
        	        data.put(param_certId, CertUtil.getCertIdByKeyStoreMap(certPath, certPwd));
                    data.put(param_signature, SDKUtil.signRsa2(data, certPath, certPwd, encoding));
                    return data;
        		}
        	}
            logger.error("未实现签名方法, version=" + version + ", signMethod=" + signMethod);
        ...
        try{
        	if(VERSION_5_1_0.equals(version)){
	            if (SIGNMETHOD_SHA256.equals(signMethod)) {
	                data.put(param_signature, SDKUtil.signSha256(data, secureKey, encoding));
	                return data;
	            } else if (SIGNMETHOD_SM3.equals(signMethod)) {
	                data.put(param_signature, SDKUtil.signSm3(data, secureKey, encoding));
	                return data;
	            }
        	}
            logger.error("未实现签名方法, version=" + version + ", signMethod=" + signMethod);
	...
	public static boolean validateBySecureKey(Map<String, String> data, String secureKey, String encoding) {
        ...
        try {
            ...
            logger.error("validateBySecureKey Error", e);
            return false;
        }
    }
	...
	public static Map<String,String> post(
		...
		logger.info("请求银联地址：" + reqUrl + "，请求参数：" + reqData.toString());
		if(reqUrl.startsWith("https://") && !UnionPayAutoConfiguration.getConfig().isIfValidateRemoteCert()) {
			reqUrl = "u" + reqUrl;
		}
		try{
	...
	public static String get(String reqUrl, String encoding) {
		...
		logger.info("get请求银联地址：" + reqUrl);
		if(!UnionPayAutoConfiguration.getConfig().isIfValidateRemoteCert()) {
			reqUrl = "u" + reqUrl;
		}
		try{
	...
	public static String deCodeFileContent(Map<String, String> data,String fileDirectory,String encoding) {
		...
		String fileContent = data.get(param_fileContent);
	...
	public static String getCustomerInfoWithEncrypt(Map<String,String> customerInfoMap,String accNo,String encoding) {
		...
		for(Iterator<String> it = customerInfoMap.keySet().iterator(); it.hasNext();){
			...
			if(key.equals("phoneNo") || key.equals("cvn2") || key.equals("expired")){
				encryptedInfoSb.append(key).append(EQUAL).append(value).append(AMPERSAND);
			}else{
				if(key.equals("pin")){
					if(null == accNo || "".equals(accNo.trim())){
				...
				sf.append(key).append(EQUAL).append(value).append(AMPERSAND);
			}
		}
		...
		if(!encryptedInfoSb.toString().equals("")){
			...
			sf.append("encryptedInfo").append(EQUAL).append(encryptData(encryptedInfoSb.toString(), encoding));
		}else{
	...
	public static String getCardTransData(Map<String, String> cardTransDataMap,
		...
		if(cardTransDataMap.containsKey("track2Data")){
			...
			track2Buffer.append(requestData.get("merId"))
					.append(COLON).append(requestData.get("orderId"))
					.append(COLON).append(requestData.get("txnTime"))
					.append(COLON).append(requestData.get("txnAmt")==null?0:requestData.get("txnAmt"))
					.append(COLON).append(cardTransDataMap.get("track2Data"));
	...
	public static int updateEncryptCert(Map<String, String> data) {
		return SDKUtil.updateEncryptCert(data.get(param_encryptPubKeyCert), data.get(param_certType));
	}
	...
	public static String getKVField(Map<String, String> map){
		StringBuffer sf = new StringBuffer();
        String info = sf.append(LEFT_BRACE)

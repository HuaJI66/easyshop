...
public class CertUtil {
    ...
    private static void initSignCert() {
        signCerts.clear();
        String path = UnionPayAutoConfiguration.getConfig().getSignCertPath();
        String pwd = UnionPayAutoConfiguration.getConfig().getSignCertPwd();
        if (isEmpty(path) || isEmpty(pwd)) {
            logger.warn(SDKConfigProperties.SDK_SIGNCERT_PATH + " or " + SDKConfigProperties.SDK_SIGNCERT_PWD + " is empty");
            return;
        }
    ...
    private static void initMiddleCert() {
        String path = UnionPayAutoConfiguration.getConfig().getMiddleCertPath();
        if (isEmpty(path)) {
            logger.warn(SDKConfigProperties.SDK_MIDDLECERT_PATH + " is empty");
            return;
        }
    ...
    private static void initRootCert() {
        String path = UnionPayAutoConfiguration.getConfig().getRootCertPath();
        if (isEmpty(path)) {
            logger.warn(SDKConfigProperties.SDK_ROOTCERT_PATH + " is empty");
            return;
        }
    ...
    private static void initTrackKey() {
        ...
        String modulus = UnionPayAutoConfiguration.getConfig().getEncryptTrackKeyModulus();
        String exponent = UnionPayAutoConfiguration.getConfig().getEncryptTrackKeyExponent();
        if (isEmpty(modulus) || isEmpty(exponent)) {
            logger.warn(SDKConfigProperties.SDK_ENCRYPTTRACKKEY_MODULUS + " or " + SDKConfigProperties.SDK_ENCRYPTTRACKKEY_EXPONENT + " is empty");
            return;
        }
    ...
    private static void initValidateCertFromDir() {
        ...
        String dir = UnionPayAutoConfiguration.getConfig().getValidateCertDir();
        if (isEmpty(dir)) {
    ...
    private static void initEncryptCert() {
        String path = UnionPayAutoConfiguration.getConfig().getEncryptCertPath();
        if (isEmpty(path)) {
            logger.warn(SDKConfigProperties.SDK_ENCRYPTCERT_PATH + " is empty");
            return;
        }
    ...
    private static void initPinEncryptCert() {
        String path = UnionPayAutoConfiguration.getConfig().getPinEncryptCertPath();
        if (isEmpty(path)) {
            logger.warn(SDKConfigProperties.SDK_PINENCRYPTCERT_PATH + " is empty");
            return;
        }
        X509Certificate encryptCert = readX509Cert(path);
    ...
    public static int resetEncryptCertPublicKey(String strCert) {
        ...
        final String localCertPath = UnionPayAutoConfiguration.getConfig().getEncryptCertPath();
    ...
    public static int resetPinEncryptCertPublicKey(String strCert) {
        ...
        final String localCertPath = UnionPayAutoConfiguration.getConfig().getPinEncryptCertPath();
        if (isEmpty(localCertPath)) {
            logger.error("未配置加密证书路径，无法执行此方法。");
            return -1;
        }
...
//    }
    ...
    private static boolean verifyCertificate(X509Certificate cert) {
        ...
        if (UnionPayAutoConfiguration.getConfig().isIfValidateCNName()) {
            // 验证公钥是否属于银联
